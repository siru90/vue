...
<div class="order_list_div" style="position: absolute;width: 100%">
	...
	<div class="feedback">
		...
		<div class="standard_con">
			<li :class="num == index ? 'active' : ''" v-for="(item, index) in data" :key="index" @click="num = index; categoryName = item.name" v-model="categoryName">{{ item.name }}<i class="icon njfont nj-xuanzhong"></i></li>
		</div>
		<p>描述</p>
