          ...
          <router-link :to="{path:login ? '/wbNotice' : '/login'}">
            <li class="aui-list-item">
              ...
              <div id="new_message" class="aui-list-item-inner aui-list-item-arrow">
                ...
                <div class="aui-list-item-right" v-if="wbdata">
                  <div class="aui-badge" style="position:relative;top:0; left:0">{{wbdata}}</div>
                </div>
                <p v-else>0</p>
              </div>
            </li>
...
<script>
  ...
  import { getWbxfList } from '../../containers/Wbxf/http'
  ...
  export default {
    ...
    methods: {
      ...
      async getInstallfn(){
        ...
        let [ref, rep,rewb] = await Promise.all([getTerminalMaintenanceinfo(0),getTerminalDismantleinfo(0),getWbxfList(1)])
        ...
        try {
          let wbnum = 0
          rewb.data.data[0].infolist.forEach(item => {
            if (item.maintenanceservicetype == 0 && item.maintenancestatuscode <=11 && item.maintenancestatuscode >=0) {
              wbnum++
            } else if (item.maintenanceservicetype == 1 && item.maintenancestatuscode >=0 && item.maintenancestatuscode <= 9) {
              wbnum++
            }
          });
          this.wbdata = wbnum
