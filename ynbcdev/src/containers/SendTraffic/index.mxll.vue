<template>
	<div>
		<div class="vue_app" style="position: absolute;width: 100%">
			...
			<div class="centent" v-if="" style="position:relative;margin-top: 2.5rem;">
								...
								<li class="aui-list-item" v-for="(item,index) in flowInitData" :key="index" style="padding-left:0.75rem;background: none;border-top: 0;">
									...
									<div class="aui-media-list-item-inner">
										...
										<div class="aui-list-item-inner" style="width: 100%;padding-bottom: 0;">
											<p style="padding-bottom: 0.5rem;">{{getTime()}}时间：{{getTimeNewDate(item.receivetime)}}</p>
											<div style="background: #FFFFFF;width: 100%;position: relative;">
												<div style="padding: 0.3rem;">
													<p><span class="getFlowTitle">{{flowName()}}：</span><span class="getFlowContent">{{item.receiveflow}} m</span><span v-if="activeNum == 2" class="getFlowContent">/{{item.remainFlow}}m</span></p>
													<p v-if="item.ifcharge == 1 || activeNum == 3 || activeNum == 2">终端编号：{{item.terminalno}}</p>
													<p v-if="item.ifcharge == 1 || activeNum == 3 || activeNum == 2">车牌号码：{{item.plateno}}</p>
												</div>                   
												<!--//ifcharge是否充值 0 未充值  1 已充值  isvalid  1是有效  0是无效-->
												<span class="addMoneyBtn" v-if="item.ifcharge == 0 && activeNum != 3 && item.isvalid == 1" @click="toSendMoney(item)">充值</span>
											</div>
										</div>
									</div>
								</li>
...
//					console.log(this.monthFlowList,'这是子的季节')
					...
					this.initData = this.fullSendList	
				}else{
			...
			//显示不同的时间前缀
			getTime(){
				if(this.activeNum == 0 || this.activeNum == 2){
					return '签到';
			...
			//显示流量信息
			flowName(){
				if(this.activeNum == 0 || this.activeNum == 1){
					return '获得流量';
				}else if(this.activeNum == 3){
					return '本月已用流量/可用流量';
