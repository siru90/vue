<template>
  <div style="height:100%">
          ...
          <div id="draw-bg" class="info primary-background-color">
            <div class="thumb" data-status="online">
              <div style="overflow: inherit;" class="avatar">
                <!--to="/main/profile"-->
                <div v-if="userInfo && userInfo.vipheadphoto" tag="div" style="
    ...
    background-position: center;"
            ...
            <div style="padding-right:0;float:left;width:auto;" class="data" id="suneee-side-nav-showuser">
              <div v-if="userInfo">
                <h4 id="userid_h4" >{{userInfo.vipname}}</h4>
                <h5 id="userid_h5" style="color:#ffffff" >{{secretPhone}}</h5>
              </div>
          ...
          <ul>
            <li class="background-transparent-darker-hover" v-for="(item, index) in menuCofig" @click="jumpClick(item)" :key="index">
              <a class="open-room" :class="item.class" :title="item.name">
                ...
                <span class="name"><i class="fa" :class="item.iconStyle"></i> &nbsp{{item.name}}</span>
              </a>
            </li>
            <li id="backToServeApp" @click="backToServeApp" class="background-transparent-darker-hover">
              <a class="open-room class_a"  >
                  <i style="color:#ffffff" class="njfont nj-fanhui" aria-label=""></i>
                  <span style="color:#ffffff" class="name">返回服务商</span>
                </a>
            </li>
          </ul>
    ...
    <div class="" style="width:100vw;height:100%;overflow:auto;">
      <header class="aui-bar aui-bar-nav aui-bar-light opacity_title" style="position:fixed"
        ...
        <div v-if="headerConfig.handlePart.type == 2" class="aui-pull-right text-del"
             @click="headerConfig.handlePart.handleClick">删除
        </div>
      ...
      <!-- 给header站位 -->
      <div id="headerC" style="height:3rem;width:100%"></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      backToServeApp() {
        // let host = window.api.mall_sale_host
        // const url = host.substr(0,host.length-1)
        window.location.href = window.api.SALE_WEB + 'sale/H5/mobile/yineng_mobile/fws/index.html?sessionId=' + window.localStorage.getItem("sessionId")
      },
      animateNum () {
