<template>
  <div class="order_list_div" style="position: absolute;width: 100% ;top:2.75rem;">

    <!--头部-->
    <repair-head style="background:#ffffff;position:fixed;top:2.75rem;width:100%;z-index:999;border-bottom:1px solid #ededed;" if="headList" :titleInfor="headList">
        ...
        <transition-group :name="animateClassName">
          <ul class="aui-list aui-media-list commodity_list subtotal_list" v-for="(item, index) in data" :key="index">
            <li class="aui-list-item">
                <div class="aui-list-item-inner">
                  <div class="aui-list-item-text">
                    <div class="aui-list-item-title"> <span style="background:#970000;color:#ffffff;borderRadius:2px;padding:1px 3px;">信</span> 订单编号：{{item.orderNo}}</div>
            ...
            <li class="edit_address">
              ...
              <router-link :to="{name:'ios_pay',params:{money:item.realPay,orderNo:item.parentOrderNo}}">
...
<script>
  import {getAllOrder, cancelOrder, confirmOrder, getUserInfoAgain, getOrderResultCheckInfo} from "../../components/config/getData.js"
  import {mapGetters, mapMutations, mapActions} from "vuex"
  import {MessageBox, TabContainer, TabContainerItem} from 'mint-ui'
  import {Toast} from '../../utils/toast2.js'
  import RepairHead from "../../components/repairHead"
...
//    },
    ...
    async mounted() {
      ...
      document.querySelector('body').id = "proOrder"
      this.SET_HEADER({
        titlePart: {
          isHome: false,
          title: '全部采购单'
...
<style>
  #proOrder .aui-bar.aui-bar-light{
      border-bottom: 0 !important;
  }
  #proOrder .slide-fade-enter-active {
    transition: all .18s ease;
  }

  #proOrder .slide-fade-leave-active {
    transition: all .18s ease;
  }

  #proOrder .slide-fade-enter, .slide-fade-leave-to {
    transform: translateX(100%);
  }
  #proOrder .slide-left-enter-active, #proOrder .slide-right-leave-active,#proOrder .slide-left-leave-active,#proOrder .slide-right-enter-active{
  	    transition: all .18s ease;
  }
  #proOrder .slide-left-enter, #proOrder .slide-right-leave-to{
  	transform: translateX(100%);
  }
  #proOrder .slide-left-leave-to, #proOrder .slide-right-enter{
	transform: translateX(-100%);
  }
  #aLink{
    color: #0f8de0;
