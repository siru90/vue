<template>
    ...
    <div id="newnewHome" @load="contentLoad">
      <!--<mt-popup
      ...
      <div id="cjyHomeOuter"
        ...
        <div>        <!-- 搜索框部分 -->
          ...
          <div class="home_swiper home_margin_bottom">
            <swiper :options="swiperOption" ref="mySwiper2" v-if="swiperSlides">
              <swiper-slide v-if="item.enable == '1' " v-for="(item,index) in swiperSlides" class="aui-slide-node bg-dark"
                            style="height:23.33vh;overflow: hidden " :key="index">
                <img :src="item.image"
                    class="swiper_img"
          ...
          <div class="classification">
            <div class="aui-row">
              <div v-if="cateList" style="width:20%;" class="aui-col-5" v-for="(item, index) in cateList" :key="index">
                <!--/-->
                ...
                <div style="position:relative">
                    ...
                    <img style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);max-width:80%;" v-lazy="item.image " @click='jumpEvent(item)'/>
                </div>
          ...
          <div class="notice_con aui-content aui-margin-b-15">
            <ul style="height:1rem; overflow:hidden;margin:0 4vw;background:#e4e4e4" class="aui-list aui-list-in">
              <li style="min-height:1rem; padding-left:0" class="aui-list-item" tapmode>
                <div style="height: 1rem;padding-left:2vw;padding-right:12vw" class="aui-list-item-label-icon">
                  <!-- <i class="icon njfont nj-gonggao"></i> -->
                  <div class="line-notice">|</div>&nbsp;
                  <span style="font-size: 12px;color:#b50101" >公告</span>：
                </div>
                <div class="aui-list-item-inner" style=";height: 1rem;">
          ...
          <div class="hot-sales">
            <div class="hot-sales-title">
                <i class="icon njfont nj-ego-hot" style="font-size: 0.6rem;"></i>
                热销TOP 10
            </div>
            <div class="hot-sales-warp">
              <div :style="hotSalesWarp"  class="hot-sales-content">
                <div @click="gotoGoodsDetailPage(item.goodscode)" v-for="item in hotSalesData" :key="item.goodsId" class="hot-sales-item">
                  ...
                  <p class="goodsPrice">{{item.discountprice}}元</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="guess-like">
            <div class="guess-like-title" style="position:relative">
              <i class="icon njfont nj-cainixihuan" style="font-size: 0.6rem;"></i>
                猜你喜欢
            ...
            <div class="guess-like-content">
              <div @click="gotoGoodsDetailPage(item.goodscode)" v-for="item in guessLikeData" :key="item.id" class="guess-like-item">
                ...
                <p class="goodsName">{{item.goodsname}}</p>
                <div class="goodsPrice">{{item.discountprice}}元
                  <!-- <div class="add-goods-to-car">
                    <i class="icon njfont nj-gouwuche"></i>
                    
                  </div> -->
                  <cartNum :gwGoodsItem="item" :goodsId="item.goodsid"></cartNum>
          ...
          <div class="xiangpu-server">
            <div class="hot-sales-title">
                <i class="icon njfont nj-gou" style="font-size: 0.6rem;"></i>
                <span>
                  象谱·全程无忧
                </span>
...
//        console.log(this.priceFontSize)
    ...
    mounted () {
      ...
      //     container: document.querySelector('#cjyHomeOuter'), //下拉容器
      //     triggerDistance: 100,
      // },function(ret) {
      //     if(ret.status=="success"){
      //       that.pullStatus = true
      //       that.refreshHomeData(() => {
      //         pullRefresh.cancelLoading();//刷新成功后调用此方法隐藏
      //         that.pullStatus = false
      //       });     
      //     }
