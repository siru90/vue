<template>
     <div v-if="checkGoodType(gwGoodsItem)">
        ...
        <div class="lijigoumai" @click="gotoConfirmorderPage(gwGoodsItem)">立即购买</div>
    </div>
    <div v-else>
...
<script>
    ...
    import {getServiceData} from "./getServiceData.js";
	export default {
		props: {
            goodsId: {
                type: Number,
       ...
       async gotoConfirmorderPage(item) {
           if (!item.stockCount) {
                MessageBox({
                    title: "商品调货中...",
        ...
        getCategory(code) {
            ...
            async add() {
                ...
                console.log("添加购物 …………………………………………………………………………");
                let serveId = ""
                let resServiceData =  await getServiceData(this.gwGoodsItem.goodsCode)
                if (resServiceData.data.returnCode === 1) {
                    try {
                        let serveArr = []
                        let allServiceData = resServiceData.data.data[0]
                        allServiceData.forEach(item => {
                            let f = true
                            for (let i = 0; i < serveArr.length; i++) {
                                const e = serveArr[i];
                                if (e.servetype === item.servetype) {
                                    f = false
                                }
                            }
                            if (f) {
                        ...
                        serveArr.forEach(v => {
                            let tmpOjb = {
                                category: this.getCategory(v.servetype),
                                id: v.serveid,
                                name: v.servename,
                                price: v.serveprice,
                                serveno: v.serveno,
                        ...
                        serveId = JSON.stringify(tmpArr)
                    } catch (error) {
                        console.log(error)
                        console.log("加入购物车 尝试获取绑定的虚拟商品失败")
                    }
                }
                //判断有无库存
