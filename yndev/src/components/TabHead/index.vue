...
; "></div>
        ...
        <!--每个分类选项按钮-->
        <li :key="index" class="cjy-tab-item"  v-for="(item, index) in titleInfor"
      ...
      <!--下拉菜单按键-->
      <div v-if="titleInfor.length > 4" class="classify_select" style="height: 2rem;position:absolute!important;"
           @click="classifyClum = true">
        <em></em>
      </div>
      <!--下拉菜单按键end-->
    </div>
    ...
    <transition name="nav-window">
      <div class="select_con" style="display: block;overflow: hidden;z-index: 88;position: absolute;top: 0"
           v-show="classifyClum"
           @click="classifyClum = false">
        <h3 style="font-size: 0.7rem">分类<em></em></h3>
        <ul class="aui-row">
          <li :key="index" class="aui-col-xs-3" v-for="(item , index) in titleInfor" @click="changeIndex(index)"><span
            :class="{'active-list':activeList == index}">{{item.goodsClassName}}</span></li>
