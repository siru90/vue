...
//                   if (true) {
                    ...
                    this.rechargeSuccessfullyTimer = setInterval(() => {
                        if (this.rechargeSuccessfullyTime === 0 || this.rechargeSuccessfully < 1) {
                            ...
                            if (s === 1 || this.sbFirst) {
                                ...
                                this.$router.replace({name: "sendTraffic", query: {num: 3}});
                            }
                            //this.$store.state.jyModules
                            this.$store.dispatch('changeClassifyIndex',3)
            ...
            gotoRechargePage() {
                this.$router.push({path: "/main/trafficTopUp", query: {firstCZ:1,item: {receiveflow: this.options.number, ...this.options.rechargeParams}}})
            },
            //弹出提示框
            async rToast(opt,firstCz) {
                if (firstCz == 1) {
                    this.sbFirst = true
                }
