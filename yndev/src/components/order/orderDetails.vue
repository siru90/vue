<template>
	...
	<div class="centent" >
		...
		<div class="aui-content">
					...
					<!-- <div class="pic aui-list-item-inner"><img src="../../../static/image/demo1.png" height="300" width="300"><span>12件商品</span></div> -->
						<div class="aui-list-item-inner">
							<div class="aui-list-item-text">                                             <!--{{item.orderNo}}-->
									...
									<div class="aui-list-item-right aui-font-size-14 btn" ref="mycopy" :data-clipboard-text="arr.parentOrderNo"><div @click="getRef" class="aui-btn aui-btn-outlined">复制</div></div>
							</div>
			...
			<div  v-for="(item,index) in arr.childOrderList" :key="index" class="server_show">
				...
				<ul class="aui-list aui-media-list commodity_list subtotal_list aui-margin-b-15" v-for="(detail, hh) in item.orderDetails" :key="hh">
					...
					<div :class="{server_show:hasSubGoods(detail)}" class="server_show server_show_1">
						<div style="padding: 0.2rem 1.5rem 0.2rem 0.75rem;font-weight:bold;" v-if="detail.goodsType == 3">
							组合商品所包含商品
						</div>
						<div id="service_select" class="gift_list" v-for="(goods,index) in detail.subGoodsList" :key="index">
							<dl>
								<dd>
									<font>{{goods.goodsName}}</font>
									<!-- <span>¥{{Number(goods.goodsPrice).toFixed(2)}}x{{goods.goodsNum*detail.goodsNum}}</span> -->
									<span>x{{goods.goodsNum*detail.goodsNum}}</span>
				...
				<li class="aui-list-item">
					<div class="aui-list-item-inner">
						...
						<div>
	                        ...
	                        <!--<div @click="starCopys" class="aui-btn aui-btn-outlined">复制</div>-->
	                    </div>
					
						<!--<p>发票信息</p>
						<p>发票类型：<span>{{arr.invoice.invoiceType == 1 ? '纸质发票' : (arr.invoice.invoiceType == 2 ? '电子发票' : arr.invoice.invoiceType == 3 ? '增值税发票' : '')}}</span></p>
						<p v-if="arr.invoice.invoiceTitle == '单位'">发票抬头：<span>{{arr.invoice.companyName}}</span><span class="aui-font-size-14 text_c_gray"></span></p>
	               		<p v-if="arr.invoice.invoiceTitle == '单位'">企业税号：<span>{{arr.invoice.taxpayerNo}}</span></p>
	               		
	               		<p v-if="arr.invoice.invoiceTitle == '个人'">姓&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名：<span>{{arr.invoice.personalName}}</span></p>-->
					</div>
				</li>
			</ul>
			...
			<ul class="aui-list aui-media-list" v-if="getOrderBill.length == 0 && types">
				<li class="aui-list-item" style="padding-right: 0.5rem;">
					...
					<!--<span v-if="types && Number(realPay).toFixed(2) != 0" style="float: right;">-->
...
//						  	this.deteles();
	...
	//	                });
		...
		async isInstallInfo(){
			...
			//组合商品判断是否存在虚拟商品
			let getShopOneRecordById = axios({  //维保单详情接口
				url: window.api.local_host + '/maintenanceProgramme/getShopOneRecordById' + "?enterpriseid="+window.api.enterpriseid+"&billno="+this.orderID,
				headers: {
			...
			let selectSubGoodsDetail = axios({  //预约单详情接口
				url: window.api.local_host + '/maintenanceAdd/selectSubGoodsDetail/' + this.orderID,
				headers: {
					'sessionId': window.localStorage.getItem('sessionId'),
					'Content-Type': 'application/json',
					'Accept': 'application/json'
			...
			let [res2,req2] = await Promise.all([getShopOneRecordById,selectSubGoodsDetail]);
			...
			res2 = res2.data;
			req2 = req2.data;
			if((res2.data.length && res2.returncode) || req2.data.length){
				this.protocolType = "1";
				console.log("this.protocolType="+this.protocolType);
				
				if(res2.data.length && res2.returncode){
					this.maintenanceid = res2.data[0].spFxMaintenance.maintenanceid;
					this.maintenanceno = res2.data[0].spFxMaintenance.maintenanceno;
					this.WbOrder = 1;
				}
				else if(req2.data.length){
					this.maintenanceid = req2.data[0].info[0].maintenanceid;
					this.maintenanceno = req2.data[0].info[0].reservationno;
					this.WbOrder = 0;
				}
			}
		},
		async turnInstall() {
			if(this.protocolType == "1"){ //组合商品判断是否存在虚拟商品
				//console.log("---maintenanceno--"+this.maintenanceno);
				//console.log("--maintenanceid---"+this.maintenanceid);
				//console.log("--WbOrder---"+this.WbOrder);
				this.$router.push({name:"wbxfWorkOrderDetail", query: {maintenanceno:this.maintenanceno, maintenanceid:this.maintenanceid, isWbOrder:this.WbOrder}})
			}else{  
				if (this.protocol == 1 || this.protocol == 2 || this.protocol == 3) {
					this.$router.push({path:'/main/install',query:{serviceNo:8,parentNo:this.arr.parentOrderNo}});
					return
				}
