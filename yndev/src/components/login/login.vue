				...
				cordova.exec(
					function(response){
						...
						.then(req => {
							...
							if(location.href.indexOf("apptype") !== -1){
								let newUrl = `${that.getHttp()}://` + window.location.host + "/static/YN/H5/index.html?apptype=0#/main/profile";
								window.location.href = newUrl;
								return
							} else {
								let newUrl = `${that.getHttp()}://` + window.location.host + "/static/YN/H5/index.html#/main/profile";
								window.location.href = newUrl;
							}
						})
					...
					if(res.returncode === 1) {
						...
						let callback = data => {
							...
							console.log("登录同步购物车信息，重新渲染购物车成功");
