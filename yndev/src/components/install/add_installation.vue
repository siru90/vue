<template>
  <div id="addinstall" class="vue_app" style="position: absolute;width: 100%">
    <!--头部-->
    <header class="aui-bar aui-bar-nav aui-bar-light" style="display:block;">
      ...
      <div style="margin-top:0" class="aui-pull-right text-del" @click=" insertOrder" v-if="serviceParam == 8">保存</div>
    </header>
    <!--头部-->
    <div class="aui-content top_div">
      <ul class="aui-list aui-media-list address_list top_div">
        ...
        <router-link :to="{path:'/main/car', query:{serviceNo:serviceParam, serviceId:serviceId,addressDetails: addressDetails,carId:carId,memo:serviceDescrible,orderDate:subscribeDate ? getTime(subscribeDate) : null,sub:sub,order:order,username:username,phone:phone,address: address,serverType:serverType,type:type,maintenancetype:maintenancetype}}">
      ...
      <router-link :to="{path:'/main/serviceInfo',query:{serviceNo:serviceParam,addressDetails: addressDetails,carId:carId,memo:serviceDescrible,orderDate:subscribeDate ? getTime(subscribeDate) : null,sub:sub,order:order,serviceId:serviceId,username:username,phone:phone,address:address,serverType:serverType,type:type,maintenancetype:maintenancetype}}">
      ...
      <ul class="aui-list aui-form-list aui-margin-b-15">
        ...
        <li class="aui-list-item">
          <div class="aui-list-item-inner">
            ...
            <div class="aui-list-item-input">
              <span v-if="serviceParam !=8">{{serviceParam == '0' ? '安装单服务' : (serviceParam == '1' ? '配件安装单服务' : (serviceParam == '2' ? '维修预约' : (serviceParam == '3' ? '保养预约' : (serviceParam == '4' ? '终端维修' : (serviceParam == '5' ? '终端拆除' : '')))))
              }}</span>
              <select :disabled="type" v-else v-model="serverType">
                <option value="0">
                   请选择预约类型
                </option>
                <option value="1">
        ...
        <li class="aui-list-item">
          <div class="aui-list-item-inner">
            <div style="width:45%" class="aui-list-item-label">预约服务日期</div>
            <div @click="openTimePicker" class="aui-list-item-input">
              ...
              <span style="color:#424242" v-if="subscribeDate">{{dateToString(subscribeDate)}}</span>
  ...
  export default {
    ...
    async mounted() {
      ...
      try {
        ...
        if (this.$route.query.orderDate) {
          //this.subscribeDate = dayjs(decodeURIComponent(this.$route.query.orderDate)).subtract(8,'hour').toString()
          let dayT = this.$route.query.orderDate
          var a = dayjs(dayT).subtract(8, 'hour')
          this.subscribeDate = a.toDate()
        }
      } catch (error) {
      ...
      this.serverType = this.$route.query.serverType || 0
      this.maintenancetype = this.$route.query.maintenancetype
      // 说明从立即预约过来
      if (this.$route.query.type) {
        if(this.$route.query.type == "pj") {
          // 配件安装
          this.serverType = 1
        } else {
          if (this.$route.query.maintenancetype == 1) {
            // 保养
            this.serverType = 2
          } else {
            // 维修
      ...
      setTimeout(() => {
          const yearLine = document.querySelector("#addinstall .mint-datetime .picker-slot.picker-slot-center")
          if (yearLine) {
    ...
    methods: {
        ...
        let service_config;
  		...
  		let req = res.data.distanceList;
    		if (this.$route.query.serviceId) {
              ...
              this.serviceContactor = i.contactor
    		    }
    		  }
    		} else {
          console.log(req[0])
           ...
           this.serviceContactor = req[0].contactor
    		}
      ...
      async updateInstall () {
        ...
        if(this.serviceParam==1 || this.serviceParam ==8){
          data.usestatus = 0;
        }
        let req;
        if (data.installtype == 8) {
          data.installtype = 1
        }
        if(this.sub == 0 ){
