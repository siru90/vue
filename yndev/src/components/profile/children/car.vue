...
<template>
	<div class="vue_app" style="position: absolute;width: 100%">
		...
		<!--</router-link>-->
			...
			<div style="margin-top:0;" id="add_address" class="aui-pull-right text-del" @click.stop="toAddCar" v-if="$route.query.serviceNo != 4 && $route.query.serviceNo != 5">新增车辆</div>
			<!--</router-link>-->
		</header>
		...
		<div class="aui-content top_div">
			<ul class="aui-list aui-media-list address_list">
				<li class="aui-list-item" v-for="(myCar,index) in carArr" :key="index" @click="carToAddInstall(myCar)">
					<div class="aui-media-list-item-inner">
						<div style="margin-top: 0.4rem;" class="aui-list-item-media">
						...
						<div class="aui-list-item-inner">
							...
							<p>车辆类型：{{myCar.cartypename}}</p>
							<p>发动机号：{{myCar.engineno}}</p>
							<p>智能终端：{{myCar.termno}}</p>
...
<script>
	...
	export default  {
		...
		methods:{
			...
			goUp(){
				if (this.$route.query.serviceNo == 9) {
					this.$router.back()
					return
				}
...
//					console.log(this.$route.query.serviceNo,'bbbbbbb');
					...
					let serverType = this.$route.query.serverType
					let type = this.$route.query.type
					let maintenancetype = this.$route.query.maintenancetype
//					console.log(sub)
...
//				console.log(989898989)
				// 来自维保
				if (this.$route.query.serviceNo == 9) {
					this.$store.dispatch("jyy/selectCarItem",myCar)
					this.$router.back()
					return 
				}
				if(this.$route.query.serviceNo != undefined || this.$route.query.serviceId != undefined){
					this.$router.push({path:'/main/addInstall',query:{
						...
						maintenancetype:this.$route.query.maintenancetype
					}});
				}
