					...
					<div class="aui-card-list-content-padded">
						...
						<vue-clip :options="options" :on-sending="sending">
						    ...
						    <template slot="clip-uploader-body" scope="props">
						     	<div id="showImg">
						     	    <div class="car_img">
								      	<div id="mydiv" v-for="(file,index) in props.files" :key="index"> 
								      		<img style="width: 100%;height: 100%;" :src="file.dataUrl" v-if=" index+1 == props.files.length">
								      	</div>
										  <img :src="carObj.carphoto" alt="" style="position: absolute;right: 0.75rem;left: 0.75rem;height: 66vw;padding-right: 1.5rem;">
...
//					console.log(action,'原地');
			...
			async saveCarInfo(){
				...
				if(this.changeMyCar == "编辑车辆"){
					...
					if (!this.valiInputData().valiStatus) {
						Toast({
							message: this.valiInputData().valiMsg,
...
//									route.push({path: '/main/car', query: {serviceNo: flag}});
			...
			valiInputData() {
				if (!this.carObj.engineno) {
					return {
						valiStatus: false,
						valiMsg: "请输入发动机号！"
					}
				}
				if (this.carObj.carnumber) {
					if (this.carObj.carnumber.length > 15) {
						return {
							valiStatus: false,
							valiMsg: "车牌号码过长，请重新输入！"
						}
					}
				}
				if (this.carObj.engineno.length > 15) {
					return {
						valiStatus: false,
						valiMsg: "发动机号码过长，请重新输入！"
					}
				}
				if (this.carObj.carframeno) {
					if (this.carObj.carframeno.length > 15) {
						return {
							valiStatus: false,
							valiMsg: "车架号码过长，请重新输入！"
						}
					}
				}
				if (this.carObj.termno) {
					if (this.carObj.termno.length > 15) {
						return {
							valiStatus: false,
							valiMsg: "终端号码过长，请重新输入！"
						}
					}
				}
				return  {
			...
			async sending (file, xhr, formData) {
				...
				//	            https://files.scn.weilian.cn:40899
//								console.log("------------------------", file);
