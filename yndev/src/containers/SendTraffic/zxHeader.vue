...
; "></div>
				...
				<!--每个分类选项按钮-->
				<li :key="index" class="cjy-tab-item" v-for="(item, index) in titleInfor" :class="{'active-list':activeList == index}" @click="changeIndex(index)" ref="titleLi">{{item.goodsClassName}}
				</li>
				<!--每个分类选项按钮end-->
		...
		<transition name="nav-window">
			<div class="select_con" style="display: block;overflow: hidden;z-index: 88;position: absolute;top: 0" v-show="classifyClum" @click="classifyClum = false">
				...
				<ul class="aui-row">
					<li :key="index" class="aui-col-xs-3" v-for="(item , index) in titleInfor" @click="changeIndex(index)"><span :class="{'active-list':activeList == index}">{{item.goodsClassName}}</span></li>
