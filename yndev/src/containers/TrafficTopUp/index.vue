						...
						<li class="aui-list-item" v-for="(item,index) in initData" :key="index">
							<div class="aui-media-list-item-inner">
								<div class="aui-list-item-inner">
									<p><span v-if="item.terminalno">终端号：</span >{{item.terminalno}}</p>
									<p><span v-if="item.plateno">车牌号：</span>{{item.plateno}}</p>
								</div>
...
//						console.log(this.initData[i].terminalno,'555222')
				...
				if (this.$route.query.firstCZ == 1) {
					this.$refs.Recharge.rToast({state: 2,rechargeParams:obj},1);
				} else {
