...
import { getApiAndUiLibaray, getCoordinateByLocationName } from "../../utils/amapTools.js"
...
//				}
        ...
        async handleSubmit() {
            ...
            if(!this.phone) {
                ...
                let reg = new RegExp(/^0?(13[0-9]|15[012356789]|17[013678]|18[0-9]|14[5786]|19[9]|16[6])[0-9]{8}$/)
            ...
            try {
                const addr = document.getElementById("name").innerHTML + this.address
                await getApiAndUiLibaray()
                const p = await getCoordinateByLocationName(addr)
                positionlng = p.geocodes[0].location.lng
                positionlat = p.geocodes[0].location.lat
            } catch (error) {
                console.log("获取坐标失败",error)
                Toast({
                    duration: 2000,
                    message: "获取地址坐标失败，请输入正确的地址",
                    iconClass: "aui-iconfont aui-icon-warn"
                });
                return
            }

            this.save(positionlng, positionlat)
        },

        save(positionlng, positionlat) {
        	if(this.myPicker){
        		this.myPicker.hide ();
        	}
