<template>
    <div id="confirmorder" ref="confirmBox" class="confirm-box">
        <div class="vue_app" style="position: absolute;width: 100%" ref="confirmBox20">
            ...
            <div class="centent">
                ...
                <!--配送地址 end-->
                <div style="overflow-x:initial"  class="aui-content" v-if="pageType === 0">
                    <ul class="aui-list aui-form-list aui-margin-b-15">
                        <li class="aui-list-item">
                            <div class="aui-list-item-inner">
                                <div v-if="bindUnrealGoods" class="aui-list-item-label">服务方式<font class="required">*</font></div>
                                <div v-else class="aui-list-item-label">配送方式<font class="required">*</font></div>
                                
                                <div class="aui-list-item-input">
        
                                    <label v-if="!bindUnrealGoods" v-for="(item, index) in logisticsTypeData" :key="index" class="logistics-type">
                                    ...
                                    <!-- 绑定了虚拟商品固定为自提 -->
                                    <label v-if="bindUnrealGoods" class="logistics-type">
                                        <!-- <input class="aui-radio" type="radio" name="demo1" checked>
                                        <span>自提</span> -->
                                        <input class="aui-radio" type="radio" value="1" v-model="serviceTypeT" name="serviceTypeT" checked >
                                        <span>进店</span>
                                        <input class="aui-radio" type="radio" value="0" v-model="serviceTypeT" name="serviceTypeT" >
                                        <span>上门</span>
                                    </label>
                                    <span v-if="!logisticsTypeData.length">暂无数据1</span>
                                </div>
                            </div>
                        </li>
                        <!-- 自提时间 -->
                        <li class="aui-list-item" v-show="psState === 1">
                            <div class="aui-list-item-inner">
                                <div class="aui-list-item-label">
                                    <span v-if="bindUnrealGoods">预约时间</span> 
                                    <span v-else>自提时间</span>
                                    <font class="required">*</font>
                                </div>
                                <div @click="openTimePicker" class="aui-list-item-input">
                                    <!-- <input style="line-height: 2.2rem;" type="datetime-local" placeholder="选择日期" :value="psDate" @change="dateChange"> -->
                                    <span v-if="psDateM">{{dateToString(psDateM)}}</span>
                                    <span style="color:#999999" v-else>选择日期</span>
                                    <!-- <Date-picker v-model="psDateM" :options="options3" type="datetime" placeholder="选择日期和时间" style="width: 200px"></Date-picker> -->
                                </div>
                            </div>
                        </li>
                    </ul>
                    <!-- 选择自提服务商 -->
                    <ul class="aui-list aui-media-list top_div" v-show="psState === 1" @click.stop="chooseProvider">
                        <li v-if="bindUnrealGoods" class="aui-list-header">
                            选择服务商
                        </li>
                        <li v-else class="aui-list-header">
                            选择自提服务商
                        </li>
                        
                        <li id="select_service_provider" class="aui-list-item" style="display: flex;align-items: center;">
                            <!--选择服务商信息，显示请选择-->
                            <div class="aui-media-list-item-inner aui-list-item-arrow" v-show="providerInfo" style="flex:1;">
                                <div class="aui-list-item-inner">
                                    <div class="aui-list-item-text">
                                        <div class="aui-list-item-title">
                                            <strong>服务商：{{provider.name}}</strong>
                                            <span class="aui-font-size-14 text_c_gray"></span>
                                        </div>
                                    </div>
                                    <p>地址：{{provider.address}}</p>
                                    <p>联系人：{{provider.contactor}}</p>
                                    <p>联系电话：{{provider.telepone}}</p>
                                </div>
                            </div>
                ...
                <div class="aui-content choose_list">
                    <ul class="aui-list aui-media-list">
                        <li style="background-image:none" class="aui-list-item iscroll_list" @click.stop="showDetail">
                                <div class="aui-media-list-item-inner">
                                    <!-- 每个商品item -->
                                    ...
                                    <div class="yajin" v-show="mYHChoose === '终端优惠券' && (yajinEnable || suocheEnable)">
                                        ...
                                        <div class="yajin-bottom">
                                            ...
                                            <div v-if="suocheEnable && !yajinEnable">
                                                <label><input type="radio" name="yajin" class="suoche" @click.stop="yajinEvent(0)" checked disabled/>同意锁车防拆</label>
                                            </div>
                                            <!-- 2、支付押金 -->
                                            <div v-if="!suocheEnable && yajinEnable">
                                                <label><input type="radio" name="yajin" class="suoche" @click.stop="yajinEvent(1)" checked disabled/>支付押金</label>
                                            </div>
                                            <!-- 3、都有 -->
                                            <div v-if="suocheEnable && yajinEnable">
                                                <label><input type="radio"  class="suoche" @click.stop="yajinEvent(0)" :checked="protocolType === 0" />同意锁车防拆</label>
                                                <label><input type="radio"  class="suoche" @click.stop="yajinEvent(1)" :checked="protocolType === 1" />支付押金</label>
                                            </div>
                                            <div class="tongyi-box">
                                                <label><input type="checkbox" class="xieyi" @click.stop="tongyiEvent($event)" checked="true"/>同意推广协议</label>
                ...
                <div class="aui-content aui-margin-b-15">
                    <ul class="aui-list aui-list-in">
                        <!-- 选择优惠券 -->
                        <li class="aui-list-item aui-list-item-arrow" @click.stop="chooseYHFn">
                            <div class="aui-list-item-inner">
                                <div class="aui-list-item-title">优惠券</div>
                                <div class="aui-list-item-right">{{mYHChoose === "普通优惠券" ? "已选用" : mYHChoose}}</div>
                            </div>
                        </li>
                        <li class="aui-list-item">
                            <div class="aui-list-item-inner">
                        ...
                        <li class="aui-list-item aui-list-item-arrow">
							<div class="aui-list-item-inner" @click.stop="choseBill">
								<div class="aui-list-item-title">发票</div>																											<!--ticketList-->
								<div class="aui-list-item-right" >
									<!-- <span style="color: #EA3939;padding-right: 0.1rem;" v-if="getTicketList.length === 0 && couponCount">{{couponCount}}</span> -->
									{{this.BillBtns?"电子发票":"不开发票"}}
								</div>
							</div>
						</li>
                        <li class="aui-list-item">
...
import api from "../../api";
import { Popup ,MessageBox,DatetimePicker } from 'mint-ui';
import { deepCopy } from '../../utils/deepcopy'
import Datepicker from 'iview/src/components/date-picker';
import 'iview/dist/styles/iview.css';
import dayjs from 'dayjs'
import { getCoordinateByLocationName, getDistanceByCoordinates } from '../../utils/amapTools.js'
export default {
    props: {
    ...
    data() {
        return {
            ...
            bindUnrealGoods: false,
            psDateM: dayjs().add(1, "hour").toDate(),
            tempShopCar:null// 临时购物车
        }
    },
    computed: {
        serviceType() {
            return this.$store.state.jyy.serviceType
        },
        //满送流量具体
        flowActSendValue() {
            console.log(this.flowActRulea)
            let list = this.flowActRulea
            ...
            for (let i=0;i<list.length;i++) {
                ...
                if (element.line <= this.payPrice) {
                    return element.earning
                }
            }
        ...
        //获取当前页面类型，是虚拟商品(1)还是普通商品(0),组合商品（3）
        pageType() {
            return parseInt(this.$route.query.pageType);
        },
        buyNow (){
            if(1==this.$route.query.buyNowFlag)
                return true;
            else
             return false; 
        },
        ...
        //最终支付价格
        payPrice() {
            console.log("totalPrice:"+this.totalPrice+"yjPrice:"+this.yjPrice+"yhPrice:"+this.yhPrice+"freight:"+this.freight);
        ...
        //判断有无服务商
        providerInfo() {
            return Object.keys(this.provider).length;
        },
        // 用户地址信息
        addressShow() {
            let callback = o => {
            ...
            } else if (this.userInfo.addressList && this.userInfo.addressList.length > 0) {
                let obj = {};
                let exits = false;
...
//          	this.longs =  Math.abs(Number(this.starty) - Number(this.endy));
        ...
        getGoodsData() {
            if (this.goodsIds) {
                ...
                let callback = data => {
                    console.log("获取对应订单数据***************", data);
                    //保存到vue中
                    this[types.ORDER_GOODS](data);
                    if(this.buyNow){
                        this[types.TOTAL_PRICE]( parseFloat( data[0].wsprice)*this.goodsNum);
                    }
                }
...
//          }
            ...
            // if (this.psState === 1 && Object.keys(this.provider).length > 0 && this.psDate) {
            if (this.psState === 1) {
                if (Object.keys(this.provider).length === 0) {
                    this.errToast("请选择服务商");
                    return;
                }
                if (!this.psDate) {
                ...
                //判断自提时间是否合法
                //选择的时间
                if (new Date().getTime() > this.psDate.getTime()) {
                    this.errToast("所选日期必须大于当前日期");
                    return;
                }
                params = Object.assign({}, params, {
                    selfExtract: {
                        extractTime: dayjs(this.psDate).format('YYYY-MM-DDTHH:mm:ss[Z]'),
                        facilitatorAddress: this.provider.address,
                        facilitatorCode: this.provider.serviceno,
                        facilitatorId: this.provider.serviceid,
                        facilitatorName: this.provider.name,
                        linkman: this.provider.contactor,
                        linkmanPhone: this.provider.telepone
                    },
                });
            }
            if (this.bindUnrealGoods) {
                params.serviceType = Number(this.serviceType)
                if (!this.provider.coordinates) {
                    this.errToast("服务商数据错误");
                    return
                }
                try {
                    const reciverLocationObj = await getCoordinateByLocationName(params.deliveryAddress)
                    const reciverLocation = [reciverLocationObj.geocodes[0].location.lng, reciverLocationObj.geocodes[0].location.lat]
                    const servicerLocation = this.provider.coordinates.split(",")
                    const distanceM = await getDistanceByCoordinates(reciverLocation, servicerLocation)
                    params.mileage = (distanceM / 1000).toFixed(2)
                } catch (error) {
                    this.errToast("计算距离失败");
                }                
            }
            showLoading();
        ...
        clearVuex() {
            ...
            this.$store.dispatch("jyy/saveServiceType", 1)
        },
        resetShopCart(){
            if(this.tempShopCar!=null){
               this[types.CART_ARRAY_CHANGE]( this.tempShopCar);
               this[types.CART_NUM_CHANGE](this.tempShopCar.length);
               this[types.COMCART_ARRAY_CHANGE](null);
               this.tempShopCar=null;
            }
        },
        //重新初始化购物车
        initCart(callback2) {
        ...
        goodsImg(goodsItemData) {
            if (goodsItemData && goodsItemData.urllist) {
                if(goodsItemData.urllist[0].imgurl) {
                    return goodsItemData.urllist[0].imgurl;
                } else {
        ...
        chooseProvider() {
            if(this.buyNow) {
                let providerServiceInfo = [];
                this.orderGoods[0].subgoodsservicelist.forEach(item => {
                    providerServiceInfo.push({
                    name : item.servicename,
                    address: item.serviceidadr,
                    contactor:item.linkman,
                    telepone:item.telphone,
                    serviceid:item.serviceid,
                    serviceno:item.serviceno,
                    coordinates: item.coordinates
                });
               });
               let s=providerServiceInfo;
               this.$router.push({name:"chooseprovider", params: {s}})
            }
            else{
                let itemlist = [];
               this.orderGoods.filter(item => {
    ...
    created() {
        if(this.buyNow)
        {
            this.tempShopCar=JSON.parse(JSON.stringify(this.cartArray));
            this[types.CART_ARRAY_CHANGE]( this.comCartArray);
            this[types.CART_NUM_CHANGE](this.comCartArray.length);
            // let hasVirGood=false;
            this.cartArray.forEach((item, index) => {
                let subGoods=JSON.parse(item.serveId);
                 let hasVirGood=false;
                 for (let item1 of subGoods) {
                     if(item1.goodsType==1){
                         this.bindUnrealGoods = true;
                         break;
                     }
                 }
             }); 
            // console.log("组合商品bindUnrealGoods："+this.bindUnrealGoods);
            // console.log(" 购物车："+JSON.stringify(this.cartArray)+"组合购物车："+JSON.stringify(this.comCartArray)+" 临时保存购物车："+JSON.stringify(this.tempShopCar));
        }
         
        // 如果绑定了虚拟商品 则只允许使用自提        
        let checkGoodsArr = this.cartArray.filter((item, index) => {
            return item.goodsChecked;
        })
        for (const item of checkGoodsArr) {
            if (item.serveId && JSON.parse(item.serveId).length > 0 ) {
                this.goodsBindunreal()
            }
        ...
        this.invoice_context = this.$store.state.hzh.BillContent;
        this.BillBtns = this.$store.state.hzh.BillBtn;
        console.log("哪个xx乱写类型啊："+ this.BillBtns)
        
        // 打个补丁，有点蛋疼
        if (this.BillBtns instanceof Array) {
            if (this.BillBtns.length == 0) {
                this.BillBtns = false
            }
        } else if (this.BillBtns === "") {
            this.BillBtns = false
        }
    ...
    mounted() {
        ...
        if (this.psDate) {
            this.psDateM = this.psDate
        }
        
        
        // this.initStyle();
        setTimeout(() => {
            const yearLine = document.querySelector("#confirmorder .mint-datetime .picker-slot.picker-slot-center")
            if (yearLine) {
                yearLine.style.display = "none"
            }
        }, 16);
        this.serviceTypeT = this.serviceType
    },
    components: {
    ...
    watch: {
        ...
        serviceTypeT(v) {
            this.$store.dispatch("jyy/saveServiceType", v)
        },
        serviceType(v) {
            if (v === "0") {
                MessageBox.alert("您已选择上门服务方式，收货地址即为上门服务地址，请确认")
            }
        }
    },
    beforeDestroy() {
