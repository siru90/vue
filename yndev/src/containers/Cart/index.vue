<template>
	 <div style="height:100vh;width:100vw;display:flex;flex-flow:column nowrap;">
        ...
        <!--购物车是空的 end-->
		<div v-if="cartArray.length" class="settle_accounts fixed_div" style="position:static;display:flex;">
		...
		methods: {
			...
			getYHPrice() {
				...
				this.cartArray.forEach(item => {
					if (item.goodsChecked || item.goodsChecked == undefined) {
						if (item.goodsType === api.normalGoodsType && this.getGoodsItemData(item.goodsId)) {
							price = price + parseFloat(this.getGoodsItemData(item.goodsId).normalprice) * item.goodsNum;
