			...
			<div style="" class="addresseeMsg">			
				...
				<ul class="addresseeMsg-form">
					<li>
						...
						<span style="float:right">{{addresseeMsg.returnlinkman}}</span>
					</li>
					<li>
						<span style="float:left">联系电话</span>
						<span style="float:right">{{addresseeMsg.returntelephone}}</span>
					</li>
					<li>
						<span style="float:left">回寄地址</span>
						<span style="float:right">{{addresseeMsg.returnaddress}}</span>
					</li>
				</ul>
			</div>
...
<script>
	...
	import { getaddresseeMsg } from './http'
	export default {
		props: {
		...
		created() {
			//初始化表单验证的配置
			// this.initValidator();
			getaddresseeMsg(this.$route.query.serviceNo)
			.then(res => {
				
				if (res.data.returnCode === 0) {
					this.failToast("获取收件人信息失败")
					return
				}
				console.log(res.data)
				this.addresseeMsg = res.data.data[0]
		...
		mounted() {
			// 设置背景，前置页面的背景被修改过
			document.querySelector('#app').style.backgroundColor = 'rgb(237, 237, 237)';
		},
		beforeDestroy() {
