    ...
    <div v-if="commodityDetail">
      ...
      <div class="goods_detail">
        ...
        <div class="goods_title">
          ...
          <div class="aui-row">
            <span v-if="!parseInt(commodityDetail.goodstype) && (parseInt(commodityDetail.wsprice) < 299)" style="font-size:0.6rem"
              class="aui-col-xs-4">运费：￥{{ !parseInt(commodityDetail.goodstype) && (parseInt(commodityDetail.wsprice) < 299) ? freight : "0"}}</span>
            <span v-else style="font-size:0.6rem;color:#EA3939" class="aui-col-xs-4">
              <span style="color: #A9A9A9">运费：</span>免运费
            </span>
          </div>
        </div>
        ...
        <div v-if="commodityDetail.subgoodslist" class="aui-content aui-margin-b-15 ">
          <div class="aui-list aui-list-in groupGoods">
            ...
            <div class="groupGoodsList">
              <div :style="hotSalesWarp(commodityDetail.subgoodslist.length)"  class="hot-sales-content">
                <div @click="gotoGroupGoodsDetailPage(item.goodsCode)"  v-for="(item, index) in commodityDetail.subgoodslist" :key="index" class="hot-sales-item">
                  <div class="goodsPic">
                  ...
                  <p class="goodsPrice"><span style="float:right;color:#666666">× {{item.goodsNum}}</span> </p>
      ...
      <div class="purchased" style="overflow: visible;z-index: 200">
        ...
        <!--<div id="add_to_car" class="aui-btn aui-btn-danger aui-btn-sm">加入购物车</div>-->
        <div class="edit_data" style="display: none;">
          <span>购买数量：</span><i class="del_cart"></i><input type="number" value="1"><i class="add_to_cart"></i>
        </div>
      </div>
      <!--购买数量 end-->
      <!--服务选择-->
...
<script>
  ...
  export default {
    ...
    methods: {
      hotSalesWarp(num) {
        return {
          'width': num * 31.13 + 4 + 'vw'
        }
      },
      gotoGroupGoodsDetailPage(goodsCode) {
        this.$router.push({path: '/groupDetail/' + goodsCode})
      },
      // 获取时间
      getNowFormatDate() {  
          var date = new Date();  
